...
class TodoManager:
    ...
    def update(self, items: list) -> str:
        log.debug("TODO  >>> items=%d", len(items))
        if len(items) > 20:
        ...
        in_prog = sum(1 for t in self.items if t["status"] == "in_progress")
        log.debug("TODO  <<< total=%d done=%d in_progress=%d pending=%d",
                  len(self.items), done, in_prog, len(self.items) - done - in_prog)
        return self.render()
    def render(self) -> str:
        if not self.items:
            return "No todos."
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        log.debug("TODO  rounds_since_todo=%d", rounds_since_todo)
        if rounds_since_todo >= 3:
            log.warning("TODO  nag injected (rounds_since_todo=%d)", rounds_since_todo)
            tool_msg = {
                        "role": "user",
